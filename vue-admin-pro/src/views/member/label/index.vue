      ...
      <u-filtered>
        <el-form :inline="true" :model="listQuery" class="search-form">
          <el-form-item label="标签名称">
            <el-input v-model="listQuery.labelName" placeholder="请输入标签名称" clearable />
          </el-form-item>
          <el-form-item>
        ...
        <template v-slot:right>
          <el-table-column label="操作" align="center" fixed="right">
            <template slot-scope="{ row }">
              ...
              <el-button type="text" @click="hdlDelete(row.id, row.labelName)">删除</el-button>
            </template>
          </el-table-column>
        </template>
      </u-table>
    ...
    <u-dialog :title="textMap[dialogStatus]" :show.sync="dialogFormVisible" @confirm="dialogStatus==='create'?hdlCreate():hdlUpdate()">
      <el-form ref="dialogForm" :rules="rules" :model="dataForm" label-width="80px">
        ...
        <el-form-item prop="labelName" label="标签名称">
          <el-input v-model="dataForm.labelName" placeholder="请输入标签名称" clearable />
        </el-form-item>
